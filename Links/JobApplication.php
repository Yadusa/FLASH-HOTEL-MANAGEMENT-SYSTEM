...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_FILES['cv_file']) && $_FILES['cv_file']['error'] === 0) {
        ...
        $file_name = time() . "_" . preg_replace("/[^a-zA-Z0-9.]/", "_", $_FILES['cv_file']['name']);
        $cv_path = $upload_dir . $file_name;

        if (move_uploaded_file($_FILES['cv_file']['tmp_name'], $cv_path)) {
            
            // B. If upload succeeds, save to Database
            $full_name    = htmlspecialchars($_POST['full_name']);
            $email        = htmlspecialchars($_POST['email']);
            $phone_number = htmlspecialchars($_POST['phone_number']);
            
            // Convert positions array to a string (e.g., "Kitchen, Management")
            $positions_list = isset($_POST['positions']) ? implode(", ", $_POST['positions']) : "None selected";

            $stmt = $conn->prepare("INSERT INTO job_applications (full_name, email, phone_number, cv_path, positions) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("sssss", $full_name, $email, $phone_number, $cv_path, $positions_list);

            if ($stmt->execute()) {
                $show_popup = true;
            } else {
                $error_msg = "Database error: Could not save application.";
            }
            $stmt->close();
        ...
        $error_msg = "Please upload a valid CV file.";
    }
}
$conn->close();
?>
...
<head>
    ...
    <style>
        ...
        .popup-card { background: white; padding: 40px; border-radius: 20px; text-align: center; max-width: 450px; width: 90%; box-shadow: 0 10px 25px rgba(0,0,0,0.2); }
